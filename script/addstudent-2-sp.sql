...
/****** Object:  StoredProcedure [dbo].[spAddStudent]    Script Date: 02-06-2020 20:16:21 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	DECLARE @scopeUserId INT
	SET @ReturnUserId = 0
	SET @ReturnCode = '0'
	IF(@StudentId <> 0)
	BEGIN
		SET @scopeUserId =  (select UserId from Students where StudentId =  @StudentId)
		IF EXISTS (SELECT 1 FROM Users Where Email = @Email AND UserId <> @scopeUserId )
		BEGIN
			SET @ReturnCode = '2'
			RETURN
		END
	...
	BEGIN
		...
		BEGIN
			INSERT INTO 
				...
				VALUES (@FirstName, @LastName, @Email, @Email)
